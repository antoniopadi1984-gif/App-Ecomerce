...
export default function AiBridgePanel() {
 ...
 AI <span className="text-rose-600 not-italic">COLLAB</span> BRIDGE
 </h1>
 ...
 <Card className="xl:col-span-8 rounded-[3rem] border-none shadow-sm bg-white overflow-hidden">
 ...
 <Card className="rounded-[3rem] border-none shadow-sm bg-rose-600 text-white p-8">
 ...
 <p className="text-[10px] font-bold text-rose-100 mt-2 opacity-80 leading-relaxed uppercase">Manual-Assisted Automation: El puente inyecta el contexto en tu cuenta abierta.</p>
 </div>
 <div className="space-y-3">
 <Button className="w-full bg-white text-rose-600 hover:bg-slate-50 font-black rounded-2xl h-14 gap-3 uppercase text-[10px] tracking-widest shadow-sm" onClick={() => window.open('https://gemini.google.com', '_blank')}>
...
function TaskCard({ title, desc, icon: Icon, onAction }: any) {
 ...
 <div className="h-10 w-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-600 group-hover:bg-rose-600 group-hover:text-white transition-colors">
 <Icon className="h-5 w-5" />
 </div>
 <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 hover:text-rose-600" onClick={(e) => { e.stopPropagation(); onAction(); }}>
 ...
 <div className="mt-4 flex items-center gap-2 text-rose-600 font-black text-[9px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
...
function BridgeHistoryItem({ title, model, time }: any) {
 ...
 <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-300 hover:text-rose-600">
