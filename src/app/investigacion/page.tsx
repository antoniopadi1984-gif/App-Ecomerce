...
import { Microscope, Play, CheckCircle2, Circle, Loader2, ArrowRight, BrainCircuit, ScanSearch, UserPlus, FileText, Share2, Target } from 'lucide-react';
import { AgentCompanion } from '@/components/layout/agent-companion';

const TABS = [
    { id: 'CORE', label: 'God Tier Research' },
    { id: 'AVATARES', label: 'Avatares (Macro)' },
    { id: 'ANGULOS', label: 'Angle Engine' },
    { id: 'COMBOS', label: 'Combo Matrix' },
];
...
function PillTab({ active, label, set }: { active: boolean; label: string; set: () => void }) {
    return (
        <button
            ...
            style={active ? { '--tab-color': 'var(--inv)' } as any : {}}
        >
            {label}
        </button>
    );
...
// ─── PÁGINA PRINCIPAL /investigacion ────────────────────────────────
export default function InvestigacionPage() {
        ...
        fetch(`/api/research/god-tier?productId=${productId}`)
            ...
            .then(d => {
                if (d.ok && d.runs) {
                    ...
                    if (runsArr.length > 0) {
                        // Ordenados por latest created at
    ...
    const startPipeline = async () => {
        ...
        for (const step of STEPS) {
            ...
            try {
                ...
                if (!d.ok) break; // Terminar si hay error
    ...
    const contextForAgent = `Investigación (Research Lab). Viendo pestaña ${activeTab}. Tienda: ${activeStoreId}. Producto filtrado: ${productId}`;
    ...
    return (
        <div className="content-main flex flex-col gap-4 pt-0 h-full">
            ...
            {!activeStoreId || productId === 'GLOBAL' ? (
                <div className="text-center p-8 text-[11px] font-semibold text-[var(--text-dim)] ds-card flex flex-col items-center justify-center min-h-[400px]">
                    <Microscope size={32} className="mb-4 opacity-50" />
                    Selecciona una <b>Tienda</b> y un <b>Producto Específico</b> en la barra superior para iniciar el ciclo de investigación.
                </div>
            ) : activeTab !== 'CORE' ? (
                <div className="text-center p-8 text-[11px] font-semibold text-[var(--text-dim)] ds-card flex flex-col items-center justify-center min-h-[400px] border-dashed border-2 bg-transparent">
                    <h3 className="text-[14px] font-[800] text-[var(--text)] mb-1">Módulo Interno de Lectura</h3>
                    <p className="max-w-md">Tab dedicada a explotar el modelo generado tras la ejecución del God Tier Pipeline y visualizarlo de forma amigable (Avatares, Ángulos y Matrices procesadas).</p>
                </div>
            ) : (
                <div className="flex-1 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {STEPS.map((step, idx) => {
                            ...
                            return (
                                ...
                                >
                                    {isActive && (
                                        <div className="absolute top-0 left-0 w-full h-[2px] bg-[var(--inv)] animate-pulse" />
                                    )}
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div>
                                            {isCompleted ? <CheckCircle2 size={16} className="text-[var(--s-ok)]" /> :
                                                isActive ? <Loader2 size={16} className="text-[var(--inv)] animate-spin" /> :
                                                    <Circle size={16} className="text-[var(--border-high)]" />}
                                        </div>
                                    </div>
                                    ...
                                    <p className="text-[10px] text-[var(--text-muted)] min-h-[30px] leading-snug">
                                        {step.desc}
                                    </p>
                    ...
                    {/* Logs Históricos de Investigación */}
                            ...
                            <div className="space-y-2">
                                {historyRuns.map((runSteps, idx) => (
                                    <div key={idx} className="flex justify-between items-center text-[11px] p-2 hover:bg-[var(--surface2)] rounded-[var(--r-sm)] font-mono border border-transparent hover:border-[var(--border)] transition-colors">
                                        <div className="flex items-center gap-3">
                                            <div className="w-2 h-2 rounded-full bg-[var(--s-ok)]"></div>
            ...
            {/* Agente Compañante Inyectado, pide el role de invesa */}
