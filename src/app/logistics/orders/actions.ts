...
export async function syncShopifyProducts() {
    try {
        ...
        if (!shopifyConn || !shopifyConn.apiKey || !shopifyConn.extraConfig) {
        ...
        for (const sp of shopifyProducts) {
            ...
            const product = await (prisma as any).product.upsert({
                ...
                update: {
                    title: sp.title,
                    handle: sp.handle,
                    image: sp.image?.src || (sp.images?.[0]?.src) || null,
                    updatedAt: new Date()
                },
                create: {
                    storeId: store.id,
            ...
            if (sp.variants && sp.variants.length > 0) {
                const firstVariant = sp.variants[0]; const price = parseFloat(firstVariant.price) || 0;
...
export async function setProductProvider(productId: string, providerName: string) {
    try {
        ...
        if (!store) {
            try {
                store = await (prisma as any).store.create({ data: { name: 'Default Store', currency: 'EUR' } });
            }
            catch (e) {
                console.error("Failed to create default store", e);
                // If concurrent creation, try finding again
                store = await (prisma as any).store.findFirst();
            }
        }
...
export async function repairMonthOrderItems(month: number, year: number) {
    try {
        ...
        const days = eachDayOfInterval({ start, end }); for (const day of days) {
            ...
            try {
                await SnapshotService.generateDailySnapshot(storeIdToUse, day, true);
            }
            catch (e) {
...
export async function upsertShopifyOrder(shopifyOrder: any, storeId: string) {
    ...
    // --- GEOCODING & RISK ---
    await autoGeocodeOrder(mainOrder.id).catch(() => { }); await calculateOrderRisk(mainOrder.id); return mainOrder;
}
...
async function getOrCreateStoreId(preferredId?: string) {
    ...
    const first = await (prisma as any).store.findFirst(); if (first) return first.id;

    // 3. Last resort: Create the default-store record
    console.log("[Store] Creating default-store record as fallback...");
    const created = await (prisma as any).store.create({
        data: { id: "default-store", name: "Tienda Principal", currency: "EUR" }
    });
    return created.id;
}
...
function extractUtms(o: any) {
    ...
    // Search keys
    const KEYS_SOURCE = ['utm_source', 'UTM source', 'source', 'fuente', 'utm_site', 'site_source_name']; const KEYS_MEDIUM = ['utm_medium', 'UTM medium', 'medium', 'medio'];
...
export async function syncOrderItemsAndProducts(shopifyOrder: any, localOrderId: string, storeId: string) {
    ...
    for (const item of shopifyOrder.line_items) {
        ...
        let unitCost = finance?.unitCost || 0; if (!finance) {
...
export async function syncShopifyHistory(storeId: string) {
    try {
        ...
        console.log(`[Prisma Debug] Action Context. Prisma URL: ${(prisma as any)._engineConfig?.datasources?.[0]?.url || 'NOT_FOUND_ON_ENGINE'}`);

        let connection;
        try {
            connection = await (prisma as any).connection.findFirst({
                where: { provider: "SHOPIFY" }
            });
        }
        catch (findErr: any) {
            console.error("[Prisma Debug] findFirst failed inside action:", findErr);
            throw findErr;
        }
        ...
        // --- 4. UPDATE ACCOUNTING SNAPSHOTS (BACKFILL HISTORY) ---
        try {
            const today = new Date(); const ninetyDaysAgo = new Date();
            ninetyDaysAgo.setDate(today.getDate() - 90);

            console.log(`[Shopify Sync] Triggering historical snapshot backfill from ${ninetyDaysAgo.toISOString()} to ${today.toISOString()}...`);

            // We do it in the background to not block the response
            SnapshotService.generateRangeSnapshots(storeIdToUse, ninetyDaysAgo, today)
                .then(() => console.log(`[Shopify Sync] Historical backfill completed.`))
...
export async function autoGeocodeAllPending(limit = 0) {
    ...
    if (count > 0) revalidatePath("/logistics/orders");
...
export async function syncRecentShopifyOrders(limit = 250) {
    try {
        ...
        try {
            const today = new Date(); await SnapshotService.generateDailySnapshot(storeIdToUse, today);
        }
        catch (snapErr) {
    ...
    try {
        ...
        await shopify.getAllDraftOrders(async (batch) => {
            ...
            for (const draft of batch) {
                ...
                const status = draft.status === "completed" ? "CONFIRMED" : "DRAFT"; const dDate = new Date(draft.created_at);
    ...
    try {
        ...
        await shopify.getAllAbandonedCheckouts(async (batch) => {
            ...
            for (const checkout of batch) {
                ...
                // unless recovered.
                const isRecovered = !!checkout.completed_at; const cDate = new Date(checkout.created_at);
...
export async function getLocalOrders(skip = 0, take = 100, sortDirection: 'asc' | 'desc' = 'desc', typeFilter?: string, dateStr?: string, productId?: string) {
    try {
        ...
        // Map to UI-friendly structure if needed, or The UI expects certain fields.
...
export async function syncDailyPerformance() {
    try {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            const dailyAdSpend = 0; await (prisma as any).dailyFinance.upsert({
...
export async function syncSingleOrderBeeping(orderId: string, apiKey?: string, apiUrl?: string) {
    try {
        ...
        // Ensure keys
        const key = apiKey || process.env.BEEPING_API_KEY; const url = apiUrl || process.env.BEEPING_API_URL;
        ...
        let details = null; if (Array.isArray(response)) {
        ...
        const rawStatus = parseInt(details.status) >= 4 ? (details.tracking_stage || details.status) : details.status; const newStatus = normalizeLogisticsStatus("BEEPING", rawStatus);
        ...
        if (newStatus !== order.logisticsStatus) {
            ...
            try {
                await SnapshotService.generateDailySnapshot(order.storeId, new Date());
            }
            catch (snapErr) {
...
async function processBeepingOrderUpdate(bo: any) {
    ...
    if (local) {
        ...
        await calculateOrderProfit(local.id); return { updated: true };
    }
...
export async function syncBeepingStatuses(limit = 0, priority = false, segment: 'ACTIVE' | 'TRANSIT' | 'FINAL' = 'FINAL') {
    try {
        ...
        const where: any = { logisticsProvider: "BEEPING" }; if (segment === 'ACTIVE') {
        ...
        const chunkSize = 100; for (let i = 0; i < pendingOrders.length; i += chunkSize) {
...
async function updateCreativePerformance(order: any) {
    ...
    const conditions = []; if (order.content) {
...
export async function updateFulfillmentRule(ruleId: string, data: any) {
    try {
        ...
        if (ruleId === 'default') {
            const firstRule = await (prisma as any).fulfillmentRule.findFirst(); if (firstRule) {
                ruleId = firstRule.id;
            }
            else {
...
export async function updateProductFinance(productId: string, data: any) {
    ...
    try {
        ...
        const sanitizedData: any = {}; for (const [key, val] of Object.entries(data)) {
            if (typeof val === 'number') sanitizedData[key] = val; else sanitizedData[key] = parseFloat(val as string) || 0;
        }
...
export async function updateOrderAddressMaster(orderId: string, addressData: any) {
    try {
        ...
        if (beepingConn && order.shopifyId) {
            try {
                const beeping = new BeepingClient(beepingConn.apiKey!, beepingConn.extraConfig!); await beeping.updateOrder(order.shopifyId, {
...
export async function getSupplyChainStats() {
    try {
        ...
        let rules = await (prisma as any).fulfillmentRule.findMany({ orderBy: { provider: 'asc' } }); try {
            ...
            distinctProviders.forEach((p: any) => {
                ...
                if (!requiredProviders.find(rp => rp.name === pName)) {
                    requiredProviders.push({ name: pName, base: 6.00 });
                    // Default base for auto-discovered
                }
            });
        ...
        const products = productsRaw.map((p: any) => {
            ...
            const stats = priceStats.find((s: any) => s.productId === p.id); let realAvgPrice = (p.finance?.sellingPrice || 0); if (stats && stats._sum.quantity > 0) {
                realAvgPrice = (stats._sum.totalPrice || 0) / stats._sum.quantity;
            }

            // 2. Infer Provider from recent history
            const pLogistics = providerItems.filter((i: any) => i.productId === p.id); const providerCounts: Record<string, number> = {};
...
export async function getDailyOperationsMatrix(month: number, year: number) {
    try {
        ...
        const startDate = new Date(Date.UTC(year, month - 1, 1, 0, 0, 0)); const endDate = new Date(Date.UTC(year, month, 0, 23, 59, 59));
        ...
        const rules = await (prisma as any).fulfillmentRule.findMany(); const defaultRule = rules[0] || { baseShippingCost: 6.5, returnCost: 3.5, codFeeFixed: 0, codFeePercent: 0 };
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const madOffset = 1;
            // Standard approximation or use complex lib
            const dayOrders = orders.filter((o: any) => {
                const oDate = new Date(o.createdAt);
                // Shift to "Local" representation
                const localDate = new Date(oDate.getTime() + (madOffset * 60 * 60 * 1000)); const oStr = localDate.toISOString().split('T')[0]; return oStr === dayStr;
            });

            const dayFinance = finances.find((f: any) => {
                // Finance is already stored as UTC midnight, so we just take the date part
                const fStr = new Date(f.date).toISOString().split('T')[0]; return fStr === dayStr;
            });
            ...
            const revenueShopify = confirmedOrders.reduce((acc: number, o: any) => acc + o.totalPrice, 0); const numConfirmed = confirmedOrders.length;
            ...
            const numShipped = numConfirmed; const deliveredCount = confirmedOrders.filter((o: any) =>
            ...
            confirmedOrders.forEach((o: any) => {
                ...
                const provider = o.logisticsProvider || "MANUAL"; const rule = rules.find((r: any) => r.provider === provider) || defaultRule;

                // OUTBOUND Shipping: Paid for ALL confirmed orders as per business rule
                const outboundBase = (rule.baseShippingCost || 0) + (rule.packagingCost || 0) + (rule.handlingCost || 0); const outboundTax = outboundBase * ((rule.taxPercent || 0) / 100);
            ...
            // Financial Metrics
            const avgOrderValue = numConfirmed > 0 ? (revenueShopify / numConfirmed) : 0; const realRevenue = deliveredCount * avgOrderValue;
            ...
            // BREAKEVEN LOGIC
            const grossProfitBeforeAds = realRevenue - totalCogs - totalShippingCost - totalReturnCost; const roasBreakeven = realRevenue > 0 && grossProfitBeforeAds > 0 ? (realRevenue / grossProfitBeforeAds) : 0;
...
export async function updateOrderNotes(orderId: string, notes: string) {
    try {
        ...
        // revalidatePath("/logistics/orders");
        // Optional if we want instant reflection without refresh
        return { success: true };
    }
    catch (e: any) {
        return { success: false, message: e.message };
...
export async function updateDailyFinance(dateStr: string, data: { adSpend?: number, visitors?: number }) {
    try {
        ...
        const inputDate = new Date(dateStr); const date = new Date(Date.UTC(
