...
export default function ProductBrain({ productId }: { productId: string }) {
 ...
 <header className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-900 p-4 rounded-lg border border-slate-800 shadow-sm gap-4">
 ...
 MEMORY GRAPH <span className="text-[8px] bg-rose-600 px-1.5 py-0.5 rounded-sm text-white">BETA ENGINE</span>
 </h2>
 ...
 <div className={`w-3 h-3 rounded-full mb-1 ${maturity?.overall > 0.7 ? 'bg-emerald-500 shadow-sm' : maturity?.overall > 0.4 ? 'bg-amber-500 shadow-sm' : 'bg-rose-500 shadow-sm'} shadow-sm animate-pulse`}></div>
 ...
 <Button onClick={handleSync} disabled={loading} variant="ghost" className="text-rose-400 hover:bg-white/5 h-7 w-7 p-0 rounded-lg">
 ...
 <BarChart3 className="w-3.5 h-3.5 text-rose-500" /> Score de Madurez
 </CardTitle>
 </CardHeader>
...
className="h-2 rounded-full bg-slate-100" />
 ...
 <h4 className="text-[10px] font-black text-rose-700 uppercase mb-1 flex items-center gap-2">
 <RefreshCw className="w-3.5 h-3.5" /> Loop de Aprendizaje
 </h4>
 <p className="text-[10px] font-bold text-rose-900/60 leading-tight">
 ...
 <CardTitle className="text-[10px] font-black uppercase text-slate-400 flex items-center gap-2">
 <GitBranch className="w-3.5 h-3.5 text-rose-500" /> Conexiones Brain Graph
 </CardTitle>
 ...
 <div key={i}
className="group p-3 bg-slate-50 hover:bg-slate-900 transition-all rounded-lg border border-slate-100 hover:border-slate-800 space-y-1.5 cursor-pointer">
 <div className="flex items-center justify-between">
 <Badge className="bg-rose-100 text-rose-600 border-0 text-[7px] font-black group-hover:bg-rose-500 group-hover:text-white uppercase px-1 py-0">
 {link.relationType}
 </Badge>
