...
export default function StaticAdsPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-12 font-sans selection:bg-yellow-200">
            ...
            <header className="relative flex items-center justify-between mb-10 z-10 shrink-0 border-b border-slate-100 pb-8">
                <div className="flex items-center gap-4">
                    <div className="h-14 w-14 bg-white rounded-2xl flex items-center justify-center shadow-premium border border-slate-200">
                        <ImageIcon className="h-7 w-7 text-yellow-600" />
                    </div>
                    <div>
                        <h1 className="text-4xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">Static Ads Lab</h1>
                        <div className="flex items-center gap-3 mt-1">
                            <Badge variant="outline" className="bg-yellow-50 border-yellow-200 text-yellow-700 px-2 py-0.5 font-bold text-[9px] uppercase tracking-widest leading-none">
                                Visual Studio v.1.0
                            </Badge>
                            <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px]">Nano Banana Graphic Intelligence</p>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-12 gap-8 relative z-10">
                ...
                <div className="col-span-12 lg:col-span-4 xl:col-span-3 space-y-6">
                    <Card className="bg-white border-slate-200 rounded-[2.5rem] p-8 space-y-8 shadow-luxe group relative overflow-hidden">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <select
                                        ...
                                        className="w-full h-12 bg-slate-50 border-slate-200 rounded-xl focus:ring-yellow-500/20 focus:border-yellow-500/50 transition-all font-bold text-xs text-slate-700 px-4 outline-none appearance-none shadow-inner"
                                    >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className="h-12 bg-slate-50 border-slate-200 rounded-xl focus:ring-yellow-500/20 focus:border-yellow-500/50 transition-all font-bold text-xs text-slate-700 px-4 shadow-inner"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {['Premium', 'Impact', 'Clean', 'Organic'].map((style) => (
                                    <Button key={style} variant="outline" className="h-12 rounded-xl border-slate-100 bg-slate-50/50 text-[10px] font-black uppercase tracking-widest hover:border-yellow-200 hover:bg-yellow-50 transition-all">
                                        {style}
                                    </Button>
                                ))}
                            </div>
                        ...
                        <Button
                            ...
                            className="w-full h-14 rounded-2xl bg-yellow-500 hover:bg-yellow-600 text-black font-black uppercase tracking-widest shadow-xl shadow-yellow-200 transition-all active:scale-95 flex items-center justify-center gap-2"
                        >
                            {isGenerating ? <RefreshCw className="h-4 w-4 animate-spin shrink-0" /> : <Wand2 className="h-4 w-4 shrink-0" />}
                            <span className="truncate text-[10px] md:text-xs">Diseñar Conceptos</span>
                        </Button>
                    </Card>
                        ...
                        <div className="h-[700px] border-2 border-dashed border-slate-200 rounded-[3rem] bg-slate-50/50 flex flex-col items-center justify-center relative overflow-hidden">
                            <div className="relative text-center space-y-4">
                                <div className="h-24 w-24 bg-white rounded-[2rem] mx-auto flex items-center justify-center shadow-2xl border border-slate-100 animate-bounce-slow">
                                    <ImageIcon className="h-12 w-12 text-yellow-700/20" />
                                </div>
                                <h3 className="text-xl font-black text-slate-950 uppercase italic tracking-tighter">Visual Intelligence Hub</h3>
                                <p className="text-slate-400 font-bold text-xs uppercase tracking-[0.3em] max-w-[350px] mx-auto leading-relaxed">Nano Banana diseña creativos estáticos que pulverizan el CTR promedio.</p>
                            </div>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
                            {concepts.map((concept, idx) => (
                                <Card key={idx} className="bg-white border-slate-200 rounded-[3rem] overflow-hidden group hover:border-yellow-200 transition-all duration-500 shadow-premium flex flex-col">
                                    <div className="aspect-[4/5] bg-slate-100 relative overflow-hidden group-hover:bg-slate-200 transition-colors">
                                        ...
                                        <div className="absolute top-6 left-6 flex flex-col gap-2">
                                            <Badge className="bg-yellow-500 text-black font-black uppercase text-[10px] w-fit">Concepto {idx + 1}</Badge>
                                            <Badge className="bg-black/80 text-white font-black uppercase text-[9px] w-fit tracking-widest">{concept.angle}</Badge>
                                        </div>

                                        <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-white via-white/80 to-transparent">
                                            <h4 className="text-2xl font-black text-slate-950 uppercase italic tracking-tighter leading-tight">{concept.headline}</h4>
                                        </div>
                                    </div>
...
function FormatSelector({ icon: Icon, label }: any) {
    return (
        <Button variant="outline" className="flex flex-col h-20 rounded-2xl border-slate-100 bg-white hover:border-yellow-200 hover:bg-yellow-50 transition-all gap-1 group">
            <Icon className="h-5 w-5 text-slate-400 group-hover:text-yellow-600 transition-colors" />
