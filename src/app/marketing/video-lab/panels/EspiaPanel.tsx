...
export function EspiaPanel({ onSelect }: { onSelect?: (item: any) => void }) {
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            <header className="glass-header flex flex-col md:flex-row md:items-center justify-between gap-3 p-3 sm:p-4 rounded-[2rem] border border-white/50 shadow-sm">
            ...
            <ScrollArea className="flex-1">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-2">
                    {loading ? (
                        ...
                        filtered.map((item) => (
                            <div key={item.id} className="group relative rounded-[2rem] border border-white/50 bg-white/20 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-transform transition-shadow duration-300">
