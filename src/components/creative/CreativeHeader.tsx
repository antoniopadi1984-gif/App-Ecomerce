...
export function CreativeHeader({ stats, onGenerate, onUpload, isGenerating }: CreativeHeaderProps) {
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/60 px-6 py-5 rounded-2xl border border-white/60 shadow-sm relative overflow-hidden glass-panel">
                <div className="absolute top-0 right-0 w-64 h-64 bg-rose-500/10 rounded-full blur-3xl -z-10 animate-pulse" />
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-rose-600 flex items-center justify-center shadow-sm animate-in zoom-in duration-500">
                        <Clapperboard className="w-6 h-6 text-white" />
                    </div>
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Creative Factory</h1>
                            <span className="px-2 py-0.5 bg-rose-50 text-rose-600 text-[9px] font-black uppercase tracking-widest rounded-full border border-rose-100">V4 Pro</span>
                        </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button
                        ...
                        className="h-10 px-6 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-black shadow-sm transition-all active:scale-95 disabled:opacity-70 group text-xs"
                    >
                        {isGenerating ? (
...
function StatCard({ label, value, sub, icon: Icon, color }: { label: string, value: string, sub: string, icon: any, color: string }) {
    ...
    return (
        <div className="bg-white/60 p-4 rounded-xl border border-white/60 shadow-sm hover:shadow-sm hover:shadow-sm transition-all duration-300 group overflow-hidden relative hover:-translate-y-1 glass-card">
