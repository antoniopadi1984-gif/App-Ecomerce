...
export class MetricsSnapshotService {
    ...
    static async patchSnapshot(storeId: string, date: Date, view: SnapshotView, block: keyof DataCompleteness, data: any) {
        ...
        if (existing?.status === 'CLOSED') {
            console.warn(`[MetricsSnapshotService] Day ${dayStart.toISOString()} is CLOSED. Update skipped.`);
            return existing;
        }
    ...
    static async syncAdsBlock(storeId: string, date: Date, view: SnapshotView, force = false) {
        ...
        if (force || !existing || isSameDay(date, new Date())) {
            try {
                ...
                console.error(`[MetricsSnapshotService] Ads Sync Failed for ${start.toISOString()}`, e);
            }
        }
    ...
    static async rebuildFullHistory(storeId: string, startStr = "2025-01-01") {
        ...
        console.log(`[MetricsSnapshotService] FULL REBUILD: Processing ${days.length} days for store ${storeId}...`);
        ...
        for (const day of days) {
            try {
                ...
                console.error(`[MetricsSnapshotService] Rebuild failed for ${day.toISOString()}:`, e);
            }
        }
        console.log(`[MetricsSnapshotService] FULL REBUILD COMPLETED.`);
    }
